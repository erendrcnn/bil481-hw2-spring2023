...
public class SuggestionEngine extends Java8BaseListener {
    ...
    public void enterMethodDeclaration(Java8Parser.MethodDeclarationContext ctx) {
        ...
        boolean isPublic = ctx.methodModifier().stream() 
                .anyMatch(modifier -> modifier.getText().equals("public")); 
				// check if the method is public
				// ctx.methodModifier() returns a list of method modifiers, so we use stream() to convert it to a stream
				// then we use anyMatch() to check if any of the modifiers is "public"

        if (isPublic) { // if the method is public, add the method name to mMethods
	...
	private TreeSet<Candidate> getTopKNeighbor(String word, int K) {
		...
		LOGGER.info("Finding the suggestions...");
		for (String methodName : mMethods) { 							// go through all methods in mMethods
            double distance = Levenshtein.distance(word, methodName); 	// compute the distance of the method name to the word
            Candidate candidate = new Candidate(methodName, distance); 	// create a new candidate
			LOGGER.info("Candidate: " + candidate.methodName + " " + candidate.distance);
            if (minHeap.size() < K) { 									// add to minHeap if it's not full
                minHeap.add(candidate);
            } else if (distance < minHeap.last().distance) { 			// replace the max element if the new one is smaller
                minHeap.pollLast();
                minHeap.add(candidate);
            }
        }

		if(minHeap.size() < K) 											// If the number of suggestions is less than K, 
		{																// print a warning message.
			System.out.println("\nUYARI: Talep edilen method onerisi sayisi bulunan method sayisindan daha fazla.\n");
		}

		String minHeapString = "[";
		for (Candidate c : minHeap) { 									// print the minHeap for debugging
			minHeapString += c.methodName + ": " + c.distance + ", ";
		}
		minHeapString = minHeapString.substring(0, minHeapString.length() - 2) + "]";
		LOGGER.info("Found the suggestions...");
		LOGGER.info("MinHeap: " + minHeapString);
